...
public class Table2wwServiceImpl implements Table2wwService {
	...
	public Table2ww update(Table2ww table2ww) throws EntityNotFoundException {
        ...
        List<Bubble> bubbles = table2ww.getBubbles();

        if(table2 != null && Hibernate.isInitialized(table2)) {
            table2.setTable2ww(table2ww);
        }

        if(bubbles != null && Hibernate.isInitialized(bubbles)) {
            if(!bubbles.isEmpty()) {
                for(Bubble _bubble : bubbles) {
                    _bubble.setTable2ww(table2ww);
                }
            }
        }
        ...
        if(bubbles != null && Hibernate.isInitialized(bubbles) && !bubbles.isEmpty()) {
            List<Bubble> _remainingChildren = wmGenericDao.execute(
                session -> DaoUtils.findAllRemainingChildren(session, Bubble.class,
                        new DaoUtils.ChildrenFilter("table2ww", table2ww, bubbles)));
            LOGGER.debug("Found {} detached children, deleting", _remainingChildren.size());
            for(Bubble _bubble : _remainingChildren) {
                bubbleService.delete(_bubble);
            }
            table2ww.setBubbles(bubbles);
        }
    ...
    @Transactional(readOnly = true, value = "bubbledataTransactionManager")
    @Override
    public Page<Bubble> findAssociatedBubbles(Integer id, Pageable pageable) {
        LOGGER.debug("Fetching all associated bubbles");

        StringBuilder queryBuilder = new StringBuilder();
        queryBuilder.append("table2ww.id = '" + id + "'");

        return bubbleService.findAll(queryBuilder.toString(), pageable);
    }
